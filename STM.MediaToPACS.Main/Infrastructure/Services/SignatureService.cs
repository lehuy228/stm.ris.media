...
namespace MediaToPacs.Infrastructure.Services
{
    public class SignatureService : ISignatureService
    {
        ...
        public async Task<List<HisUserSignatureResponse>> GetAllHisUserKySoAsync()
        {
            var response = await _httpClient.GetAsync(ApiEndpoints.Signature.HisUsers);
            response.EnsureSuccessStatusCode();
            var json = await response.Content.ReadAsStringAsync();

            return JsonSerializer.Deserialize<List<HisUserSignatureResponse>>(json,
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        }

        public async Task<HisUserSignatureResponse> GetByIdHisUserKySoAsync(Guid id)
        {
            ...
            var json = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<HisUserSignatureResponse>(json,
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        }

        public async Task<HisUserSignatureResponse> GetByHisUserKySoIdAsync(string hisUserId)
        {
        ...
        public async Task<HisUserSignatureResponse> CreateHisUserKySoAsync(HisUserSignatureRequest input)
        {
            ...
            return JsonSerializer.Deserialize<HisUserSignatureResponse>(result,
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        }

        public async Task<HisUserSignatureResponse> UpdateHisUserKySoAsync(Guid id, HisUserSignatureRequest input)
        {
            var json = JsonSerializer.Serialize(input, new JsonSerializerOptions
            {
                PropertyNamingPolicy = JsonNamingPolicy.CamelCase
